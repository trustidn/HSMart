<div>
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
        ...
        <flux:text class="text-zinc-500 dark:text-zinc-400">
            {{ 'Ringkasan tenant dan langganan.' }}
        </flux:text>

        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <flux:card>
                <flux:text class="text-zinc-500 dark:text-zinc-400">{{ 'Total Tenant' }}</flux:text>
                <flux:heading size="xl">{{ $this->adminStats['total_tenants'] }}</flux:heading>
            </flux:card>
            <flux:card>
                <flux:text class="text-zinc-500 dark:text-zinc-400">{{ 'Langganan Aktif' }}</flux:text>
                <flux:heading size="xl">{{ $this->adminStats['active_subscriptions'] }}</flux:heading>
            </flux:card>
            <flux:card>
                <flux:text class="text-zinc-500 dark:text-zinc-400">{{ 'Uji coba' }}</flux:text>
                <flux:heading size="xl">{{ $this->adminStats['trial'] }}</flux:heading>
            </flux:card>
            <flux:card>
                <flux:text class="text-zinc-500 dark:text-zinc-400">{{ 'Kadaluarsa' }}</flux:text>
        ...
        <flux:card>
            <div class="mb-4 flex items-center justify-between">
                <flux:heading size="lg">{{ 'Tenant & langganan' }}</flux:heading>
