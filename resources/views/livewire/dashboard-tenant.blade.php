<div>
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
        ...
        <flux:card>
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <flux:text class="text-zinc-500 dark:text-zinc-400">{{ 'Paket saat ini' }}</flux:text>
                    @if ($this->tenantSubscription)
                        <flux:heading size="lg">{{ $this->tenantSubscription->plan?->name ?? 'Langganan' }}</flux:heading>
                        <div class="mt-2 flex flex-wrap items-center gap-2">
                            ...
                            <flux:text class="text-sm text-zinc-500 dark:text-zinc-400">
                                {{ 'Berlaku hingga' }} {{ $this->tenantSubscription->ends_at?->format('d/m/Y') }}
                            </flux:text>
                        </div>
                    @else
                        <flux:heading size="lg">{{ 'Tidak ada langganan aktif' }}</flux:heading>
                        <flux:text class="text-sm text-zinc-500 dark:text-zinc-400">
                            {{ 'Perpanjang atau pilih paket untuk tetap menggunakan semua fitur.' }}
                        </flux:text>
                    @endif
                </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <flux:card>
                <flux:text class="text-zinc-500 dark:text-zinc-400">{{ 'Total produk' }}</flux:text>
            ...
            <flux:card>
                <flux:text class="text-zinc-500 dark:text-zinc-400">{{ 'Item stok rendah' }}</flux:text>
                <flux:heading size="xl">{{ number_format($this->tenantStats['low_stock_count'], 0, ',', '.') }}</flux:heading>
                @if ($this->tenantStats['low_stock_count'] > 0)
                    <flux:badge color="amber" class="mt-2">{{ 'Perlu perhatian' }}</flux:badge>
                @endif
            </flux:card>
            <flux:card>
                <flux:text class="text-zinc-500 dark:text-zinc-400">{{ 'Penjualan hari ini' }}</flux:text>
            ...
            <flux:card>
                <flux:text class="text-zinc-500 dark:text-zinc-400">{{ 'Pendapatan hari ini' }}</flux:text>
        ...
        <flux:card>
            ...
            <flux:table>
                <flux:table.columns>
                    <flux:table.row>
                        <flux:table.cell variant="strong">{{ 'Nomor penjualan' }}</flux:table.cell>
